...
public class PartyOrderPickupSteps {
	...
	public PartyOrderPickupSteps(TestContextSetup testContextSetup) throws IOException {
	...
	@When("I search for the store location {string} for Party Order Pickup without coupon discount")
	public void i_search_for_the_store_location_for_party_order_pickup_without_coupon_discount(String storeName)
	...
	@When("^I Click on party orders tab in store listing screen and set headcount and date and time$")
	public void i_Click_on_party_orders_tab_in_store_listing_screen_and_set_headcount_and_date_and_time()
			throws Throwable {
		testContextSetup.getStoreListPageActions().clickPartyOrderTab();
		String headCount = objDailyMenu.getJSONObject("storeDetails").getJSONObject("Rambhavan Test")
				.getString("HeadCount");

		String date = objDailyMenu.getJSONObject("storeDetails").getJSONObject("Rambhavan Test").getString("Date");

		String month = objDailyMenu.getJSONObject("storeDetails").getJSONObject("Rambhavan Test").getString("Month");

		String year = objDailyMenu.getJSONObject("storeDetails").getJSONObject("Rambhavan Test").getString("Year");

		String time = objDailyMenu.getJSONObject("storeDetails").getJSONObject("Rambhavan Test").getString("Time");
		...
		testContextSetup.getStoreListPageActions().setHeadcount(headCount);

		testContextSetup.getStoreListPageActions().setDateAndTime(date, month, year, time);
		testContextSetup.getStoreListPageActions().clickPartyOrderPopupSubmitButton();

		/*
		 * * storelistPage.txt_DateAndTime.click(); int date =
		 * objPartyMenu.getInt("Date"); int month = objPartyMenu.getInt("Month"); int =
		 * objPartyMenu.getInt("Year"); String time = objPartyMenu.getString("Time");
		 * objEcomFunctions.setDateAndTime(year, month, date, time); // 01:00 PM
		 * storelistPage.popup_btnSubmit.click();
		 */

	}

	@When("I search for a store named {string} for Party Order Pickup without coupon discount")
	public void i_search_for_a_store_named_for_party_order_pickup_without_coupon_discount(String storeName)
	...
	@When("I select the store {string} for Party Order Pickup without coupon discount")
	public void i_select_the_store_for_party_order_pickup_without_coupon_discount(String storeName)
			throws InterruptedException, AWTException {
		String expectedStoreName = objDailyMenu.getJSONObject("storeDetails").getJSONObject(storeName)
				.getString("storeName");
	...
	@When("I add the menu items for {string} to the cart for Party Order Pickup without coupon discount")
	public void i_add_the_menu_items_for_to_the_cart_for_party_order_pickup_without_coupon_discount(String string)
	...
	@When("I verify the cart information for the store named {string} for Party Order Pickup without coupon discount")
	public void i_verify_the_cart_information_for_the_store_named_for_party_order_pickup_without_coupon_discount(
	...
	@When("I proceed to checkout for Party Order Pickup without coupon discount")
	public void i_proceed_to_checkout_for_party_order_pickup_without_coupon_discount() {
		testContextSetup.getMenuPageActions().checkout();
		testContextSetup.getMenuPageActions().clickPartyOrderPopupCheckout();

	}

	@Then("I verify the order summary without discount for daily menu pickup from {string} for Party Order Pickup without coupon discount")
	public void i_verify_the_order_summary_without_discount_for_daily_menu_pickup_from_for_party_order_pickup_without_coupon_discount(
	...
	@Then("I make a payment for {string} for Party Order Pickup without coupon discount")
	public void i_make_a_payment_for_for_party_order_pickup_without_coupon_discount(String string)
			throws InterruptedException {
		Thread.sleep(1000);
...
//		    js.executeScript("arguments[0].scrollIntoView(true);", element);
	...
	@Then("I verify the order confirmation details for the {string} pickup order for Party Order Pickup without coupon discount")
	public void i_verify_the_order_confirmation_details_for_the_pickup_order_for_party_order_pickup_without_coupon_discount(
			String string) throws InterruptedException {
		Collection<MenuItem> items = testContextSetup.getCart().getCartItems();
		int index = 1;
		double itemtaxtotal = 0;
...
//need to uncomment
	...
	@Then("I login to BO and switch to the store {string} then open the order and get the status for Party Order Pickup without coupon discount")
	public void i_login_to_bo_and_switch_to_the_store_then_open_the_order_and_get_the_status_for_party_order_pickup_without_coupon_discount(
			String storeName) throws InterruptedException {
	...
	@Then("I Open new incognito browser and Login as Supervisor and open the order for {string} for Party Order Pickup without coupon discount")
	public void i_open_new_incognito_browser_and_login_as_supervisor_and_open_the_order_for_for_party_order_pickup_without_coupon_discount(
			String string) throws InterruptedException {
	...
	@Then("I confirm the order for {string} for Party Order Pickup without coupon discount")
	public void i_confirm_the_order_for_for_party_order_pickup_without_coupon_discount(String string) {
	...
	public void i_open_the_order_in_in_process_tab_and_compare_the_status_in_support_dashboard_for_party_order_pickup_without_coupon_discount()
			throws InterruptedException {
	...
	public void i_open_the_order_in_preparation_tab_and_compare_the_status_in_support_dashboard_for_party_order_pickup_without_coupon_discount()
			throws InterruptedException {
		Thread.sleep(2000);
	...
	public void i_click_on_ready_for_pickup_and_compare_the_status_in_support_dashboard_for_party_order_pickup_without_coupon_discount()
			throws InterruptedException {
...
//			driver.navigate().refresh();
	...
	public void i_click_on_delivered_and_compare_the_status_in_support_dashboard_for_party_order_pickup_without_coupon_discount()
			throws InterruptedException {
